...
vm_image = modal.Image.debian_slim(python_version="3.9.18").run_commands(
    "apt-get update",
...
@stub.function(schedule=modal.Cron("0 6 * * 1"), image=vm_image, secrets=[modal.Secret.from_name("HOPSWORKS_API_KEY")], gpu='a10g', timeout=(86399))
def train():
    import subprocess
    output = subprocess.getoutput("nvidia-smi")
    print(output)
    hopsworks_project = hopsworks.login(project='stock_news_sentiment')
